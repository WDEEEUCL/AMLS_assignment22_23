...
def extract_features_labels():
    ...
    eye_color_labels = {line.split('\t')[-1].split('.')[-2] : int(line.split('\t')[1]) for line in lines[1:]} # eye colour (0-4)
    ...
    eye_color_labels = np.array(all_labels) # simply converts the -1 into 0, so male=0 and female=1
    #return img, face_shape_labels
    print('Extraction complete')
    return img, eye_color_labels
